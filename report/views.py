...
class MonthlyStockLogs(ListView):
    model = StockOut
    template_name = 'monthly_logs.html'
    paginate_by = 50
    is_paginated = True
    ...
    def dispatch(self, request, *args, **kwargs):
        return super(MonthlyStockLogs, self).dispatch(request, *args, **kwargs)
    ...
    def get_queryset(self):
        ...
        if self.logs_month:
            self.year = self.request.GET.get('year', current_date.year) 
            ...
            queryset = StockOut.objects.filter(
                ...
                dated__month=months.index(self.logs_month) + 1, ).values('product__name').annotate(
    ...
    def get_context_data(self, **kwargs):
        ...
        if queryset:
            ...
            total_invoice=queryset.all().count()
            context.update({
